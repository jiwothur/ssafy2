App = {
  ...
  buyTokens: function() {
    ...
    App.contracts.ERC20TokenSale.deployed().then(function(instance) {
      return instance.buyTokens(numberOfTokens, {
        ...
        gas: 500000 // Gas limit 500000
      });
    }).then(function(result) {
      console.log("Tokens bought...")
      $('form').trigger('reset') // reset number of tokens in form
      // Wait for Sell event
      if ($('#rcv').prop('disabled', false)) {
  ...
  transfer: function() {
    if($('.buy').click(function() {
      ...
      App.contracts.ERC20Token.deployed().then(function(instance) {
        return instance.transfer($('#adh').text(), price, {
          from: $("#accountAddress").val(),
          gas: 500000
        });
      }).then(function(result) {
        App.transferCheck = true;
        localStorage.setItem("transferCheck", App.transferCheck);
        console.log("Success!");
        alert("Complete!");
        location.reload();
      });
...
$(function() {
    App.transfer();
    App.transferCheck = localStorage.getItem("transferCheck");
    if(App.transferCheck) {
      $('.buy').hide();
      $('.read').show();
    }
